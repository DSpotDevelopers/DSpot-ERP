...
export class AuthService extends SocialAuthService {
	...
	/**
	 * Verify OAuth token when signin with social media from DSpot ERP Teams
	 *
	 * @param provider The provider used with user for signin
	 * @param token The token generated by OAuth provider from DSpot ERP Teams frontend
	 ...
	 * This function is used to facilitate the DSpot ERP in DSpot ERP Teams try to create new Users or only signin them
	...
	async sendWorkspaceSigninCode(
		...
		try {
			...
			if (IS_DEMO) {
				const demoEmployeeEmail = environment.demoCredentialConfig?.employeeEmail || 'employee@dspot.com.pl';
				const demoAdminEmail = environment.demoCredentialConfig?.adminEmail || 'local.admin@dspot.com.pl';
